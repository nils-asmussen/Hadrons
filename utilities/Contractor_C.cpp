...
int main(int argc, char* argv[])
{
         ...
         traj < par.global.trajCounter.end; traj += par.global.trajCounter.step)
    {
        ...
        for (auto &p: par.product)
        {
            ...
            if (term.size() != addTLast.size())
            {
                HADRONS_ERROR(Size, "number of terms (" + std::to_string(term.size()) 
                            + ") different from number of addTLast components (" 
                            + std::to_string(addTLast.size()) + ")");
            }
            for (auto &s: p.times)
            {
                ...
                for (auto &dt: translations)
                {
                    ...
                    for (unsigned int tLast = 0; tLast < par.global.nt; ++tLast)
                    {
...

                       {
                          ...
                          A2AContraction::accTrMul(result.correlator[tLast], prod, ref);
                          tAr.stopTimer("tr(A*B)");
                          tflops += A2AContraction::accTrMulFlops(prod, ref);
                          tbytes += 2.*prod.rows()*prod.cols()*sizeof(ComplexD);
                       }
